...
class ModelArgs:
    def __init__(self):
        ...
        self.batch_size = 64  # 暂定，之后再看显存占用
        self.n_layer = 6
        self.vocab_size = 65
        self.n_head = 6
        self.n_embed = 384
...
class MyDataset(torch.utils.data.Dataset):
    def __init__(self, method):
        ...
        self.method = method
        if method == 'train':
            self.data = np.memmap('/gemini/code/shakespeare/train.bin', dtype=np.uint16)
        else:
            self.data = np.memmap('/gemini/code/shakespeare/val.bin', dtype=np.uint16)
